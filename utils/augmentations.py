...
class RandomHorizontalFlip(object):
    ...
    def __call__(self, coords, feats, labels):
        ...
        for curr_ax in self.horz_axes:
            ...
            coord_min = torch.min(coords[:, curr_ax])
            coords[:, curr_ax] = coord_max - coords[:, curr_ax] + coord_min
        return coords, feats, labels
...
class RandomRotate(object):
    ...
    def __call__(self, coords, feats, labels):
        ...
        z_rot_matrix = np.array([[np.cos(self.theta), -np.sin(self.theta), 0], [np.sin(self.theta), np.cos(self.theta), 0], [0, 0, 1]], dtype=np.float32)
        rot_matrix = torch.tensor(rot_matrix.dot(z_rot_matrix))
        coords[:, 1:] = coords[:,1:].matmul(rot_matrix)
        return coords, feats, labels
