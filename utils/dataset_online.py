...
class OnlineSemanticKITTIDataset(OnlineBaseDataset, ABC):
    def __init__(self,
                 ...
                 corrupt='beam_missing',
                 level='moderate',
                 corrupt_list=['beam_missing','cross_sensor','crosstalk','fog','incomplete_echo','motion_blur','snow']):
                # corrupt_list=['incomplete_echo','motion_blur','snow', 'beam_missing','cross_sensor','crosstalk','fog']):
        ...
        self.num_frames = len(os.listdir(os.path.join(self.dataset_path, self.corrupt, self.level, 'labels')))
    ...
    def get_paths(self):
        ...
        for sub_seq in self.online_keys:
            ...
            for f in frames:
                pcd_path = os.path.join(self.dataset_path, sub_seq, self.level, 'velodyne', f'{int(f):06d}.bin')
                label_path = os.path.join(self.dataset_path, sub_seq, self.level, 'labels', f'{int(f):06d}.label')
    ...
    def get_online_split(self):
        online_sequences = {x: None for x in self.corrupt_list}
        seq_len = self.num_frames // len(self.corrupt_list)

        for k, seq in enumerate(online_sequences):
            online_sequences[seq] = np.arange(k*seq_len, (k+1) * seq_len)
...
#                  corrupt='beam_missing',
#                  level='moderate',
#                  corrupt_list=['original', 'beam_missing','cross_sensor','crosstalk','fog','incomplete_echo','motion_blur','snow']):
#                 #  corrupt_list=[ 'beam_missing','cross_sensor','crosstalk','fog','incomplete_echo','motion_blur','snow']):
...
#         # 预先将 7 段索引范围存下来
#         partitions = [
#             np.arange(i * seq_len, (i + 1) * seq_len)
#             for i in range(num_partitions)
#         ]

#         # original & beam_missing 用同一批帧索引
#         online_sequences["original"] = partitions[5]
#         online_sequences["beam_missing"] = partitions[0]
#         online_sequences["cross_sensor"] = partitions[1]
#         online_sequences["crosstalk"] = partitions[2]
#         online_sequences["fog"] = partitions[3]
#         online_sequences["incomplete_echo"] = partitions[4]
#         online_sequences["motion_blur"] = partitions[5]
#         online_sequences["snow"] = partitions[6]
...
#         if not nusc:
#             self.nusc = NuScenesNew(version=self.version,
...
#         remap_dict_val = self.maps["learning_map"]   # 将不同数据集中的类别对齐
#         max_key = max(remap_dict_val.keys())
#         remap_lut_val = np.zeros((max_key + 100), dtype=np.int32)
#         remap_lut_val[list(remap_dict_val.keys())] = list(remap_dict_val.values())
...
#         lidar_file = os.path.join(self.nusc.dataroot, self.corrupt, self.level, lidar_data['filename'])
#         # print(lidar_file)
#         # input()
# #         print('lidar_file')
# #         print(lidar_file)
...
#         pcd = points[:, :3]
# #         geometric_feats = np.load(gf_path)['features']    #几何特征，需要删除
...
# #             geometric_feats = geometric_feats[range_idx]   #几何特征
...
# #                 'geometric_feats': geometric_feats}          #修改
#         data = {'points': pcd,
#                 'features': colors,
#                 'labels': points_label,
#                 'global_points': global_points}
...
#         self.geometric_path = geometric_path
        ...
        first_21 = self.token_list[:21]
        self.token_list = first_21 + self.token_list
        domain_folder_map = {
            0: 'original',        
            1: 'beam_missing',   
            2: 'cross_sensor',
        ...
        domain_idx = sequence_idx
        self.corrupt = domain_folder_map[domain_idx]
        
        total_frames = len(self.token_list)
        frames_per_domain = 21
        
        slice_start = domain_idx * frames_per_domain
        slice_end = slice_start + frames_per_domain

        total_frames = len(self.token_list)
        self.token_list = self._safe_slice(total_frames, slice_start, slice_end)


    def _safe_slice(self, total, start, end):
        start = max(0, min(start, total))
        end = max(start, min(end, total))
        return self.token_list[start:end]
...
#         self.geometric_path = geometric_path
    ...
    def get_tokens(self):
        ...
        for scene in self.sequences:
            ...
            token_list_seq = []
            names = scene.split('-')[0]
            self.names2locations[scene] = names
            scene_path = os.path.join(self.dataset_path, 'sequence'+names, 'SYNTHIA-SEQS-'+scene)
            pcd_names = os.listdir(os.path.join(scene_path, 'Lidar/Stereo_Left/Omni_F'))
            for i in range(len(pcd_names)):
                pcd_names[i] = pcd_names[i].split('.')[0]
            self.seq_token_list[scene] = pcd_names

    def set_sequence(self, idx):
        self.selected_sequence = self.sequences[idx]
        self.token_list = self.seq_token_list[self.selected_sequence]
        self.location = self.names2locations[self.selected_sequence]
    ...
    def get_frame(self, sample_token):
        ...
        # get sample record
        domain_path = os.path.join(self.dataset_path, 'sequence'+self.location, 'SYNTHIA-SEQS-'+self.selected_sequence)
        lidar_path = os.path.join(domain_path, 'Lidar/Stereo_Left/Omni_F', sample_token+'.pcd')
        label_path = os.path.join(domain_path, 'Label/Stereo_Left/Omni_F', sample_token+'.npy')

        pcd = np.asarray(o3d.io.read_point_cloud(lidar_path).points, dtype=np.float32)[:, :3]
        pcd[:,[1,2]] = pcd[:, [2,1]]
        pcd[:, 2] *= -1
        # print("1111111111")
        # print(np.min(pcd[:, 0]))
        ...
        if not os.path.exists(label_path):
            points_label = np.zeros(np.shape(pcd)[0], dtype=np.int32)
        else:
            points_label = np.load(label_path)
...
#             geometric_feats = geometric_feats[range_idx]   #几何特征
        ...
        if self.use_intensity:
            ...
            colors = np.ones((pcd.shape[0], 1), dtype=np.float32)
...
#         super(NuScenesNew, self).__init__()
        self.version = version
...
if __name__ == '__main__':
    ...
    # dataset = OnlineSynthiaDataset(sequence='01-RAIN')
    # input()
    ...
    dataset = OnlineSemanticKITTIDataset(datapath='../OpenDataLab___SemanticKITTI-C/raw/SemanticKITTI-C/')
