...
from .tent import configure_model, softmax_entropy, collect_params, T3A, PseudoLabel
...
class OneDomainAdaptation(object):
    ...
    def __init__(self,
        ...
        self.t3a = T3A(num_classes, 20, model)
        hparams = {'alpha':0.0001, 'beta':0.9, 'gamma':1, 'wdecay':1e-5}
        self.pl = PseudoLabel(num_classes, hparams, model)
...
#         _, pred_seg = self.pl(stensor, adapt=True).detach().max(1)
#         total_loss = 0
#         labels = batch['labels'].long()
        ...
        # T3A
        with torch.no_grad():
            _, pred_seg = self.t3a(stensor, adapt=True).detach().max(1)
            labels = batch['labels'].long()
            total_loss = 0
            self.optimizer.zero_grad()
        ...
        """
        # forward in mink
        out_seg = self.model(stensor, is_seg = True)
        
#         print(out_seg.shape)
#         input()
#         total_loss = softmax_entropy(out_seg).mean(0)
...
#         total_loss = self.seg_beta * loss_seg_head + ssl_loss
        ...
        # additional metrics
        _, pred_seg = out_seg.detach().max(1)
        # iou
        """
...
#         results_dict[f'{phase}/db_index'] = db_index
    ...
    def configure_optimizers(self):
        ...
        if self.use_tent:
            parameters, _ = collect_params(self.model)
        else:
...
#         self.is_pseudo = is_pseudo
    ...
    def adapt_double(self):
        ...
        # adapt
        for corrupt in self.corrupt_type:
...
#             self.reload_model()
            ...
            # set sequence in dataset, in weight path and loggers
                self.set_sequence(corrupt, sequence)
            # adapt on sequence
...
#             self.source_model.to(self.device)
        ...
        for f in tqdm(range(len(self.eval_dataset)), desc=f'Seq: {self.sequence}', leave=True):
            ...
            # get eval batch (1 frame at a time)  先对当先帧进行测试
...
#             print(batch['global_points'][0].shape)
#             print(batch['labels'])
#             import time
#             time.sleep(1)
        return batch
    ...
    def save_final_results(self):
        ...
        self.write_csv(final_dict, phase='adapt')
